export class GameView {
    ...
    init(params) {
        ...
        this.difficulty = this.app.config.difficulties[this.app.config.settings.difficulty];
        this.hints = 3;
        ...
        this.container.innerHTML = `
            <div class="flex flex-col h-screen">
                ...
                <div class="flex justify-between items-center p-2 bg-gray-800">
                    ...
                    <div class="text-white font-bold text-sm">${this.category}</div>
                ...
                <div class="flex justify-between items-center py-1 px-3 text-sm font-bold text-white bg-gray-700">
                    <div>Time: <span id="timer">${Math.floor(this.timeLeft / 60)}:${String(this.timeLeft % 60).padStart(2, '0')}</span></div>
                    <div>Tries: <span id="tries">${this.tries}</span></div>
                    <div>Found: <span id="foundCount">${this.foundWords.size}/${this.words.length}</span></div>
                </div>
                ...
                <div class="word-list grid grid-cols-2 gap-1 p-2 bg-gray-800 max-h-[25vh] overflow-y-auto">
                    ${this.words.map(word => `
                        <div class="word-item" data-word="${word}">
                            <div class="${this.foundWords.has(word) ? 
                                'bg-gradient-to-r from-green-500/20 to-green-600/20 text-green-400' : 
                                'gradient-border'} 
                                p-2 text-center text-sm ${this.foundWords.has(word) ? 'line-through' : ''}
        ...
        if (word.length > 1) {
            ...
            if (this.words.includes(word) && !this.foundWords.has(word)) {
                ...
                if (wordItem) {
                    ...
                    if (div) {
                        div.className = 'bg-gradient-to-r from-green-500/20 to-green-600/20 text-green-400 p-2 text-center text-sm line-through transition-all duration-300';
                    }
                }
    ...
    highlightCells() {
        ...
        cells.forEach(cell => {
            cell.classList.remove('bg-blue-600', 'bg-green-600');
        });
    ...
    startTimer() {
        this.timer = setInterval(() => {
            ...
            if (timerElement) {
                timerElement.textContent = `${Math.floor(this.timeLeft / 60)}:${String(this.timeLeft % 60).padStart(2, '0')}`;
            }
            if (this.timeLeft <= 0) {
    ...
    handleEnd() {
        ...
        if (selectedWord.length > 1) {
            ...
            if (this.words.includes(selectedWord) && !this.foundWords.has(selectedWord)) {
                ...
                // Update word list UI
                const wordItem = this.container.querySelector(`[data-word="${selectedWord}"]`);
                if (wordItem) {
                    const div = wordItem.querySelector('div');
                    if (div) {
                        div.className = 'bg-gradient-to-r from-green-500/20 to-green-600/20 text-green-400 p-2 text-center text-sm line-through transition-all duration-300';
